...
def tokenize_literal(literal):
    ...
    if '.' in literal:
        ...
        if not ''.join(literal.split('.')).isnumeric():
            ...
            raise Exception('User Error: Cannot have more than one "." in a literal.')
        else:
...
def perform_operation(tokens):
    ...
    if paren_start != None:
        contents = tokens[paren_start+1:i]
        if not contents:
        ...
        if len(contents_operated) == 1 and contents_operated[0]['type'] in ['integer', 'float']:
            tokens = copy.deepcopy(tokens[:paren_start]) + contents_operated + copy.deepcopy(tokens[paren_end+1:])
        else:
            tokens = copy.deepcopy(tokens[:paren_start+1]) + contents_operated + copy.deepcopy(tokens[paren_end:])
    # There are no parentheses remaining.
    else:
        tokens = perform_simple_operation(tokens)

    tokens = resolve_negatives(tokens)
    return tokens

# CONSIDER Moving the "paren" logic elsewhere.
def evaluate(text):
